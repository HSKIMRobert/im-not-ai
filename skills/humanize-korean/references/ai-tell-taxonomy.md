...
- _quick: true · quick_pattern: "(으)로 이어진다"·"~에 직결된다" 결산 + 논리 결산용 "결국" 문서 2회+ · quick_fix: 인과 경로를 구체로 쓰거나 단문 단언으로. "결국"은 1회만 남긴다_
...
- 처방: 웹 구현에서 검증된 처방을 그대로 — **효과적인 은유 1개만 남기고** 나머지는 평서 서술로("시장을 잠식한다" → "시장 점유율을 뺏는다/줄인다"). 사람 글의 살아있는 은유 1~2개는 힘이므로 보존(철칙 #9③), 원문에 없던 은유를 새로 넣는 것은 금지.
...
- _quick: true · quick_pattern: 생성형 개념 은유(잠식·청구서·과실·청사진·짊어지다·"진단은 서늘하다" 류 감각 술어)가 문서 3회+ 겹침, 또는 **같은 은유 어근('쥐다' 등)이 3회+ 관통 반복** · quick_fix: 효과적인 1개만 남기고 **직역화**(은유의 명제를 문자로: "잠식"→"점유율을 뺏는다"·"쥐다"→"가지다", 강도 유지). 중심 은유·명제 불명이면 보존. 주입 금지_
