...
DEONTIC_RE = _re.compile(
    ...
    r"|바람직하"
    r"|불가피하"
    # "짤 때다"는 앞 음절 받침이 ㄹ이라 `[을ㄹ]` 로는 안 잡힌다(정규식으로 받침만 보기 어렵다).
    # 대신 지시어(그때·이때·한때)만 제외하고 결말의 "~ 때다/때입니다"를 잡는다.
    r"|(?<![그이한])때[다입]"
    # "짤 때가 왔다/됐다"도 같은 당위다. 이걸 빼면 D-6이 지시한 **정당한 편집**이
    # 서법 소실로 오판된다 — "지금이야말로 ~할 때다"에서 상투구만 벗긴 결과가
    # 손실로 잡히고, 복원기가 "지금이야말로"를 도로 심는다(실측 3/3 재현).
    r"|때가\s*(?:왔다|됐다|되었다|온다|이다|입니다)"
    # D-6 결말 공식의 직계 변종("~해야 할 시점이다"). 카탈로그가 예문으로 명시한 형태다.
    r"|시점이[다며]|시점입니다"
    r"|촉구|당부"
    r"|하지\s*않으면\s*안\s*[되돼]"
)
